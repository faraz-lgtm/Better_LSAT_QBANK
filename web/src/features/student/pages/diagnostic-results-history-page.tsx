...
function HistoryRow({ attempt }: { attempt: GuestDiagnosticResult }) {
  ...
  return (
    <Link
      to={href}
      className="grid grid-cols-1 gap-3 rounded-[16px] border border-[#dfe1e7] bg-white p-4 transition-colors hover:bg-[#f9fbff] lg:h-[72px] lg:grid-cols-[minmax(0,1fr)_179px_40px] lg:items-center lg:gap-0 lg:p-0"
    >
      ...
      <div className="flex h-[52px] flex-col justify-center gap-1.5 rounded-[16px] border border-[#e5e7eb] bg-[#f9fafb] px-3 lg:mx-3">
        <div className="flex w-full items-center justify-between gap-2">
          <span className="text-sm font-medium leading-normal tracking-[0.02em] text-[#666d80]">Score</span>
          <span className="text-right text-sm font-semibold leading-normal tracking-[0.02em] text-[#062357]">
            {attempt.scaledScoreLabel}
          </span>
        </div>
        <p className="text-xs font-medium tracking-[0.02em] text-[#666d80]">
          {attempt.correctCount}/{attempt.questionCount} correct
        </p>
      </div>
      <div className="hidden items-center justify-center text-[#666d80] lg:flex">
  ...
  return (
    <StudentMain className="bg-[#f0f5ff]" contentClassName="flex min-h-0 flex-1 flex-col pt-6 pb-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link
          to={copy.startHref}
          className="inline-flex h-11 items-center justify-center gap-1 rounded-[12px] bg-[#0d47a1] px-5 text-sm font-semibold tracking-[0.28px] text-white hover:bg-[#0b3d8a]"
        >
          {copy.startLabel}
      ...
      <section className="mt-6 rounded-[16px] border border-[#dfe1e7] bg-white p-6 shadow-[0px_5px_5px_rgba(13,13,18,0.04),0px_4px_4px_rgba(13,13,18,0.02)]">
        <div className="mb-4 rounded-[16px] bg-[#f6f8fa] px-6 py-4">
          <h2 className="text-2xl font-bold leading-[1.3] text-[#062357]">Attempt history</h2>
        </div>
        {attempts.length === 0 ? (
