...
import { lrDrillSessionMock, type LrDrillQuestion } from "@/features/student/drills/lr-drill-mock"
...
type LrDrillQuestionPanelProps = {
  question: LrDrillQuestion
  questionNumber: number
  findQuery: string
}

function LrDrillQuestionPanel({ question, questionNumber, findQuery }: LrDrillQuestionPanelProps) {
...
function LrDrillSessionPage() {
  const questions = lrDrillSessionMock.questions
  ...
  const stimulusHit = matchesFind(current.stimulus)
        ...
        >
          ...
          <div className="grid gap-0 lg:grid-cols-2 lg:divide-x" style={{ borderColor: "var(--greyscale-100)" }}>
            <div className="max-h-[min(52vh,480px)] overflow-y-auto border-b p-5 lg:max-h-[560px] lg:border-b-0" style={{ borderColor: "var(--greyscale-100)" }}>
              <p
                ...
                style={{
                  ...
                  opacity: findQuery.trim() && !stimulusHit ? 0.45 : 1,
                }}
              >
                {current.stimulus}
              </p>
            ...
            <div className="flex max-h-[min(52vh,480px)] flex-col gap-4 overflow-y-auto p-5 lg:max-h-[560px]">
              <LrDrillQuestionPanel key={current.id} question={current} questionNumber={safeIndex} findQuery={findQuery} />
            </div>
          </div>
