    ...
    <>
      ...
      <StudentMain>
        <div className="mb-2 flex items-center justify-between">
          <button type="button" className="text-sm font-semibold text-[color:var(--color-student-accent)] hover:underline">
            {d.parentLabel}
          </button>
        ...
        <section className="rounded-2xl border border-[#dfe1e7] bg-white p-4 shadow-sm md:p-6">
          <div className="flex flex-col gap-6 lg:flex-row lg:justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-[color:var(--color-student-heading)] md:text-3xl">{d.headline}</h1>
              <p className="mt-6 text-3xl font-bold text-[color:var(--color-student-heading)]">{d.label}</p>
              <div className="mt-6 grid max-w-md grid-cols-2 gap-4 md:grid-cols-2">
                <div className="rounded-xl border border-[#efd2be] bg-[#f2e6dc] p-4">
                  ...
                  <p className="mt-2 text-sm font-semibold text-[color:var(--color-student-heading)]">Section 1</p>
                </div>
                <div className="rounded-xl border border-[#b4ddd3] bg-[#d7ebe7] p-4">
                  <p className="text-xs text-[#666d80]">RC preview</p>
                  <p className="mt-2 text-sm font-semibold text-[color:var(--color-student-heading)]">Section 2</p>
                </div>
              </div>
            </div>
            <dl className="grid grid-cols-3 gap-4 text-center lg:text-left">
              <div>
                ...
                <dd className="text-lg font-semibold text-[color:var(--color-student-heading)]">{d.questionCount}</dd>
              </div>
              <div>
                <dt className="text-xs text-[#666d80]">Total Time</dt>
                <dd className="text-lg font-semibold text-[color:var(--color-student-heading)]">{d.totalMinutes} min</dd>
              </div>
              <div>
                <dt className="text-xs text-[#666d80]">Sections</dt>
                <dd className="text-lg font-semibold text-[color:var(--color-student-heading)]">{d.sectionCount}</dd>
              </div>
            </dl>
          </div>
        </section>
        ...
        <section className="mt-6 rounded-2xl border border-[#dfe1e7] bg-white p-4 shadow-sm md:p-6">
          <h2 className="text-xl font-semibold text-[color:var(--color-student-heading)]">Test Section</h2>
          <ul className="mt-4 flex flex-col gap-3">
            {mockPrepTestSections.map((row) => (
              ...
              >
                <div>
                  <p className="font-semibold text-[color:var(--color-student-heading)]">{row.label}</p>
