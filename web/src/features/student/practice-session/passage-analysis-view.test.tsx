...
describe("PassageAnalysisBody", () => {
  it("renders paragraph labels and overall", () => {
    ...
    const explanationCard = screen.getByText("Para analysis").closest("[data-passage-analysis='explanation']")
    expect(explanationCard).toHaveClass("bg-[#F6F8FA]", "border-[#DFE1E7]", "p-6")
    expect(screen.getByText("Overall")).toBeInTheDocument()
    expect(screen.getByText("Overall takeaway")).toBeInTheDocument()
    expect(screen.getByText("Overall").closest("section")).toHaveClass("lesson-section-empty")
    expect(screen.getByText("Overall")).toHaveClass("text-white")
    const overallBody = screen.getByText("Overall takeaway").closest(".passage-analysis-overall")
    expect(overallBody).toBeTruthy()
    expect(overallBody).toHaveClass("text-white")
  })
})
