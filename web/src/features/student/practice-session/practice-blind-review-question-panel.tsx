...
function PracticeBlindReviewQuestionPanel({
  ...
  const correctIndex =
    question.correctChoiceId != null
      ? question.choices.findIndex((c) => c.id === question.correctChoiceId)
      : -1
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      <div className="shrink-0 border-b border-[#e5e7eb] bg-[#f6f8fa] p-6">
        <div
          className={cn(
            "flex gap-3",
            reviewChrome ? "flex-col items-stretch" : "items-start",
          )}
        >
          {!reviewChrome ? (
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-2">
            ...
            <div className="flex items-start gap-3">
              <PracticeAnnotatedContent
                ...
                className={cn(BLIND_REVIEW_QUESTION_STEM_CLASS, "min-w-0 flex-1")}
              />
              {reviewChrome ? (
                <button
                  type="button"
