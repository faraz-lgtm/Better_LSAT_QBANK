...
const PRACTICE_SESSION_REVIEW_PANEL_CLASS =
  "practice-session-review-panel absolute inset-x-0 bottom-0 z-20 flex h-[512px] max-h-[min(512px,75vh)] flex-col overflow-hidden rounded-t-[32px] bg-white px-[72px] pt-16 shadow-[0px_-8px_24px_rgba(13,13,18,0.08)]"
...
const PRACTICE_SESSION_REVIEW_PANEL_TITLE_CLASS =
  "text-[24px] font-bold leading-[1.3] text-[#0d0d12]"

const PRACTICE_SESSION_REVIEW_PANEL_CLOSE_BUTTON_CLASS =
  "inline-flex size-6 shrink-0 items-center justify-center text-[#666d80] transition hover:opacity-80"
...
const PRACTICE_SESSION_REVIEW_PANEL_FILTER_ITEM_CLASS =
  "inline-flex items-center gap-2 text-sm font-medium leading-[1.5] tracking-[0.28px] text-[#666d80]"

const PRACTICE_SESSION_REVIEW_PANEL_FILTER_BOX_CLASS =
  "inline-flex size-5 shrink-0 items-center justify-center overflow-hidden rounded-[6px] border border-[#dfe1e7] bg-[#f6f8fa]"
...
const PRACTICE_SESSION_REVIEW_QUESTION_BUTTON_ANSWERED_CLASS =
  "border border-[#0b4e6e] bg-[#0d47a1] text-[#f3f7ff]"

const PRACTICE_SESSION_REVIEW_QUESTION_BUTTON_DEFAULT_CLASS =
  "border border-[#d1d5dc] bg-white text-[#062357] hover:bg-[#f6f8fa]"
